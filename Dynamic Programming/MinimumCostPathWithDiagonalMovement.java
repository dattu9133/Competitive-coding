...
public class MinimumCostPathWithDiagonalMovement {
  ...
  private static int findMinimumPathSumR(int i, int j, int[][] cost) {
    ...
    int fromTop = findMinimumPathSumR(i - 1, j, cost);
    int fromLeft = findMinimumPathSumR(i, j - 1, cost);
    int fromDiagonal = findMinimumPathSumR(i - 1, j - 1, cost);

    return Math.min(Math.min(fromTop, fromLeft), fromDiagonal) + cost[i][j];
  }
}
